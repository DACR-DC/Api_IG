...
const controladorasistencia = {
  create: async (req, res) => {
    ...
    if (!req.body.id_clase || !req.body.fecha) {
      return res
        .status(400)
        .json({ message: "id_clase and fecha are required" });
    }
    ...
    asistencia.crear_asistencia(req.body, (err, result) => {
      if (err) {
        return res
          .status(500)
          .json({ message: "Error creating asistencia", error: err });
      }
      res
        .status(201)
        .json({ message: "Asistencia created", asistencia: result });
    });
  },
  ...
  obtenerTodasasistencia: async (req, res) => {
    asistencia.obtener_asistencias((err, result) => {
      if (err) {
        return res
          .status(500)
          .json({ message: "Error al obtener las asistencias", error: err });
      }
      res.json(result);
    });
  },
  ...
  obtenerasistenciaPorId: async (req, res) => {
    ...
    asistencia.obtener_asistencia_ID(id, (err, result) => {
      if (err)
        return res
          .status(500)
          .json({ message: "Error al obtener la asistencia", error: err });
  ...
  crearasistencia: async (req, res) => {
    ...
    asistencia.crear_asistencia(nuevaasistencia, (err, result) => {
      if (err) {
        return res
          .status(500)
          .json({ message: "Error al crear la asistencia", error: err });
      }
      res
        .status(201)
        .json({
          mensaje: "Asistencia creada exitosamente",
          asistenciaCreada: result,
        });
    });
  },
  ...
  actualizarasistencia: async (req, res) => {
    ...
    asistencia.actualizar_asistencia(
      id,
      ...
      (err, result) => {
        if (err) {
          return res
            .status(500)
            .json({ message: "Error al actualizar la asistencia", error: err });
        }
        if (result.affectedRows === 0) {
          return res
            .status(404)
            .json({ message: "Asistencia no encontrada para actualizar" });
        }
  ...
  eliminarasistencia: async (req, res) => {
    ...
    asistencia.borrar_asistencia(id, (err, result) => {
      if (err) {
        return res
          .status(500)
          .json({ message: "Error al eliminar la asistencia", error: err });
      }
      if (result.affectedRows === 0) {
        return res
          .status(404)
          .json({ message: "Asistencia no encontrada para eliminar" });
      }
  ...
  getAsistenciaPorClase: async (req, res) => {
    ...
    asistencia.obtener_asistencia_por_clase(
      req.params.id_clase,
      (err, result) => {
        if (err) {
          return res
            .status(500)
            .json({
              message: "Error al obtener la asistencia por clase",
              error: err,
            });
        }
        res.json(result);
  ...
  updateStatus: async (req, res) => {
    ...
    asistencia.actualizar_estado(req.body.id_clase, (err, result) => {
      if (err) {
        return res
          .status(500)
          .json({ message: "Error updating asistencia", error: err });
      }
    ...
    asistencia.actualizar_asistencia_estudiante(id, status, (err, result) => {
      if (err) {
        return res
          ...
          .json({
            message:
              "Error al actualizar el estado de la asistencia del estudiante",
            error: err,
          });
      }
      if (result.affectedRows === 0) {
        return res
          .status(404)
          .json({ message: "Asistencia del estudiante no encontrada" });
      }
      res
        .status(200)
        .json({
          message: "Asistencia del estudiante actualizada correctamente",
        });
    });
  },
  // ===================== Profesor asistencia =====================
    ...
    asistencia.obtener_asistencia_claseID(id, (err, result) => {
      if (err)
        return res
          .status(500)
          .json({
            message: "Error al obtener la asistencia de la clase",
            error: err,
          });
      if (!result || result.length === 0)
        return res
          .status(404)
          .json({ message: "Asistencia de clase no encontrada" });
      res.json(result);
    });
  },
};
