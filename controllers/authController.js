...
console.log("Contraseña almacenada (hash):", usuarioDB.contrasena);
    ...
    const [existingToken] = await db.promise().query(
    ...
    if (existingToken.length > 0) {
      ...
      return res.json({ message: 'Sesión ya iniciada', token: existingToken[0].token, user: { ...usuarioDB, contrasena: null } });
    }
...
exports.verificarToken = async (req, res, next) => {
  ...
  jwt.verify(token, JWT_SECRET, async (err, decoded) => {
    ...
    try {
      ...
      if (result.length === 0) return res.status(401).json({ message: 'Token inválido o sesión cerrada' });
